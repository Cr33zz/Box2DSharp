...
public class b2Fixture
{
	...
	{
		Console.Write("    b2FixtureDef fd;\n");
		Console.Write("    fd.friction = %.15lef;\n", m_friction);
		Console.Write("    fd.restitution = %.15lef;\n", m_restitution);
		Console.Write("    fd.density = %.15lef;\n", m_density);
		Console.Write("    fd.isSensor = bool(%d);\n", m_isSensor);
		Console.Write("    fd.filter.categoryBits = uint16(%d);\n", m_filter.categoryBits);
		Console.Write("    fd.filter.maskBits = uint16(%d);\n", m_filter.maskBits);
		Console.Write("    fd.filter.groupIndex = int16(%d);\n", m_filter.groupIndex);
		...
		case b2Shape.Type.e_circle:
		{
				...
				Console.Write("    b2CircleShape shape;\n");
				Console.Write("    shape.m_radius = %.15lef;\n", s.m_radius);
				Console.Write("    shape.m_p.Set(%.15lef, %.15lef);\n", s.m_p.x, s.m_p.y);
		}
			break;
		...
		case b2Shape.Type.e_edge:
		{
				...
				Console.Write("    b2EdgeShape shape;\n");
				Console.Write("    shape.m_radius = %.15lef;\n", s.m_radius);
				Console.Write("    shape.m_vertex0.Set(%.15lef, %.15lef);\n", s.m_vertex0.x, s.m_vertex0.y);
				Console.Write("    shape.m_vertex1.Set(%.15lef, %.15lef);\n", s.m_vertex1.x, s.m_vertex1.y);
				Console.Write("    shape.m_vertex2.Set(%.15lef, %.15lef);\n", s.m_vertex2.x, s.m_vertex2.y);
				Console.Write("    shape.m_vertex3.Set(%.15lef, %.15lef);\n", s.m_vertex3.x, s.m_vertex3.y);
				Console.Write("    shape.m_hasVertex0 = bool(%d);\n", s.m_hasVertex0);
				Console.Write("    shape.m_hasVertex3 = bool(%d);\n", s.m_hasVertex3);
		}
			break;
		...
		case b2Shape.Type.e_polygon:
		{
				...
				Console.Write("    b2PolygonShape shape;\n");
				Console.Write("    b2Vec2 vs[%d];\n", DefineConstants.b2_maxPolygonVertices);
				for (int i = 0; i < s.m_count; ++i)
				{
					Console.Write("    vs[%d].Set(%.15lef, %.15lef);\n", i, s.m_vertices[i].x, s.m_vertices[i].y);
				}
				Console.Write("    shape.Set(vs, %d);\n", s.m_count);
		}
			break;
		...
		case b2Shape.Type.e_chain:
		{
				...
				Console.Write("    b2ChainShape shape;\n");
				Console.Write("    b2Vec2 vs[%d];\n", s.m_count);
				for (int i = 0; i < s.m_count; ++i)
				{
					Console.Write("    vs[%d].Set(%.15lef, %.15lef);\n", i, s.m_vertices[i].x, s.m_vertices[i].y);
				}
				Console.Write("    shape.CreateChain(vs, %d);\n", s.m_count);
				Console.Write("    shape.m_prevVertex.Set(%.15lef, %.15lef);\n", s.m_prevVertex.x, s.m_prevVertex.y);
				Console.Write("    shape.m_nextVertex.Set(%.15lef, %.15lef);\n", s.m_nextVertex.x, s.m_nextVertex.y);
				Console.Write("    shape.m_hasPrevVertex = bool(%d);\n", s.m_hasPrevVertex);
				Console.Write("    shape.m_hasNextVertex = bool(%d);\n", s.m_hasNextVertex);
		}
			break;
		...
		Console.Write("\n");
		Console.Write("    fd.shape = &shape;\n");
		Console.Write("\n");
		Console.Write("    bodies[%d]->CreateFixture(&fd);\n", bodyIndex);
	}
